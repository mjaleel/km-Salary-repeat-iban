...
def clean_amount_val(val):
    """
    دالة تنظيف قوية جداً تتعامل مع:
    - الأرقام العربية (٠١٢٣)
    - الفواصل والنقاط
    - النصوص الغريبة
    """
    ...
    # 1. تحويل الأرقام العربية/المشرقية إلى إنجليزية
    arabic_map = {
        '٠': '0', '١': '1', '٢': '2', '٣': '3', '٤': '4',
        '٥': '5', '٦': '6', '٧': '7', '٨': '8', '٩': '9'
    }
    for arb, eng in arabic_map.items():
        val_str = val_str.replace(arb, eng)

    # 2. حذف المسافات وأي رموز عملة
    # نبقي فقط الأرقام . ,
    val_str = re.sub(r'[^\d.,]', '', val_str)

    # 3. معالجة الفواصل والنقاط
    # السيناريو العراقي الشائع: 1.250.000 (نقاط كفواصل) -> نحذف النقاط
    # السيناريو العالمي: 1,250,000 (فواصل) -> نحذف الفواصل
    
    if "." in val_str and "," not in val_str:
        # إذا كان الرقم يحتوي نقاطاً فقط
        if val_str.count(".") > 1: # مثل 1.250.000
             val_str = val_str.replace(".", "")
        else:
             # نقطة واحدة قد تكون عشرية أو فاصلة آلاف، نحاول تحويلها
             pass 

    val_str = val_str.replace(",", "") # حذف الفواصل دائماً للأمان
        ...
        with tab1:
            if st.button("بدء الفحص", key="btn_audit"):
                ...
                for index, row in df.iterrows():
                    ...
                    # نعتبر الصف فارغاً إذا كان الايبان والمبلغ فارغين
                    is_empty_row = (raw_iban == "" and amt == 0.0)
                    
                    if not is_empty_row and amt <= 0:
                        # قمت بإضافة (القيمة المقروءة) هنا لتساعدك في معرفة السبب
                        val_debug = str(row[amount_col])
                        critical_errors.append(f"❌ [صف {row_num}] المبلغ صفر أو غير صالح. (القيمة الموجودة في الملف: {val_debug})")
